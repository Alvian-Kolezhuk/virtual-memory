...
%define PT_ENTRY_SIZE_LOG 3
%define PT_ENTRY_SIZE (1 << PT_ENTRY_SIZE_LOG)
%define PAGE_MAP_LEVEL_4_ADDRESS 0xffd000
%define PAGE_DIRECTORY_POINTER_ADDRESS 0xffe000
...
%define NUMBER_OF_PAGE_DIRECTORIES ((NUMBER_OF_PAGE_TABLES + PAGE_SIZE / PT_ENTRY_SIZE - 1) >> (PAGE_SIZE_LOG - PT_ENTRY_SIZE_LOG))
%if NUMBER_OF_PAGE_DIRECTORIES > 1
  %error "Current realization does not support more than 1 page directories"
%endif
    
initialize_page_table:
; Initialize Page Map Level 4
    mov edi, PAGE_MAP_LEVEL_4_ADDRESS
    mov ecx, (PAGE_SIZE / 4)
    xor eax, eax
    rep stosd       ; repeat(ecx) mov [edi], eax; add edi, 4
    
    mov eax, PAGE_DIRECTORY_POINTER_ADDRESS
    or eax, 3       ; 1 bit -- enable write, 0 bit -- present
    mov [PAGE_MAP_LEVEL_4_ADDRESS], eax

; Initialize Page Directory Pointer
    mov edi, PAGE_DIRECTORY_POINTER_ADDRESS
    mov ecx, (PAGE_SIZE / 4)
    xor eax, eax
    rep stosd       ; repeat(ecx) mov [edi], eax; add edi, 4
    ...
    or eax, 3       ; 1 bit -- enable write, 0 bit -- present
    mov [PAGE_DIRECTORY_POINTER_ADDRESS], eax
...
; Write to CR3 PAGE_MAP_LEVEL_4_ADDRESS
    mov eax, PAGE_MAP_LEVEL_4_ADDRESS
    mov cr3, eax
...
gdt_code:
    ...
    db 0x20         ; Flag (64 bit descriptor) and limit 16:19
    db 0x00         ; Base 24:31
gdt_data:
...
long_mode_message:
    db ' tacts in long mode with page addressing', 0
